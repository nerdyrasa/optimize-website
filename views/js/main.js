...
// Get the pizzasDiv OUTSIDE of the LOOP!
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover'),
    scrollingAdjustment = document.body.scrollTop / 1250; // do calculation only once before doing looped operations

  // Create an array so that values can be looked up instead of calculated every time through the loop
  var phaseList = [];
  for (var j = 0; j < 5; j++) {
    phaseList.push(Math.sin((scrollingAdjustment) + (j % 5)));
  }
